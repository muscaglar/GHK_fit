function [params,output] = extreme_GHK_Fit_curve_fit(cIDs,saltName,polarity,dirName,plotName,sheetName,do_plot)
...
switch saltName
    case {'ALaCl3','CeCl3','LaCl3'}
                ...
                [fitresult, gof] = fit_tri(MODEL_x,MODEL_y,do_plot,initial);
        initial = [fitresult.C,fitresult.A];
        [fitresult, gof] = fit_tri(x,y,do_plot,initial);

        flag = 0;
    case   {'BKCl','KCl','LiCl'}
        
        [fitresult, gof] = fit_mono(MODEL_x,MODEL_y,do_plot,initial);
        initial = [fitresult.C,fitresult.A];
        [fitresult, gof] = fit_mono(x,y,do_plot,initial);
    ...
    case   {'HfCl4','ZrCl4'}
        ...
        flag = 1;
    case   {'MgCl2'}
                [fitresult, gof] = fit_di(MODEL_x,MODEL_y,do_plot,initial);
        initial = [fitresult.C,fitresult.A];
        [fitresult, gof] = fit_di(x,y,do_plot,initial);

        flag = 0;
    case   {'K3PO4'}
        [fitresult, gof] = fit_other_tri(MODEL_x,MODEL_y,do_plot,initial);
        initial = [fitresult.C,fitresult.A];
        [fitresult, gof] = fit_other_tri(x,y,do_plot,initial);
