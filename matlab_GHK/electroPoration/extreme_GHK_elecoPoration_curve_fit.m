function [output] = extreme_GHK_elecoPoration_curve_fit(cell_input,saltName,polarity,Origin)

no_Pairs = size(cell_input,2);
output = zeros(no_Pairs,16);
...
for i = 1:no_Pairs
    ...
    [pre_params,~] = extreme_GHK_Fit_curve_fit(cap_Pair(1),saltName,polarity,num2str(i),num2str(i),num2str(i),0);%dirName,plotName,sheetName,Origin)
    [post_params,~] = extreme_GHK_Fit_curve_fit(cap_Pair(2),saltName,polarity,num2str(i),num2str(i),num2str(i),0);%dirName,plotName,sheetName,Origin)
